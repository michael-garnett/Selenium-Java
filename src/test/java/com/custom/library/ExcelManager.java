...
public class ExcelManager {
	...
	/*
	 * @SuppressWarnings("static-access") public static void setCellData(String
	 * data, int RowNum, int ColNum) { try {
	 * 
	 * Row = ExcelWSheet.createRow(RowNum); Cell = Row.getCell(ColNum,
	 * Row.RETURN_BLANK_AS_NULL); if (Cell == null) { Cell = Row.createCell(ColNum);
	 * Cell.setCellValue(data);
	 * 
	 * FileOutputStream fileOut = new FileOutputStream(excelFile);
	 * ExcelWBook.write(fileOut); fileOut.flush(); fileOut.close(); } } catch
	 * (Exception e) { logger.error("Error: exception occured: ", e); } }
	 * 
	 * public static void setExcelFile(String sheetName) { try { //FileOutputStream
	 * file = new FileOutputStream(excelFile); ExcelWBook = new XSSFWorkbook();
